...
void UK2Node_SiriusPrintStringFormatted::AllocateDefaultPins()
{
	...
	{
		ArgumentPins.Emplace(CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Wildcard, PinName));
	}
...
void UK2Node_SiriusPrintStringFormatted::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	...
	if (Pin == FormatPin && FormatPin->LinkedTo.Num() == 0)
	{
		...
		for (const FString& Param : ArgumentParams)
		{
			...
			if (!TryFindArgumentPin(ParamName))
			{
				...
				CreatePinParams.Index = Pins.IndexOfByKey(FormatPin) + 1 + ArgumentPins.Num();
				ArgumentPins.Emplace(CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Wildcard, ParamName, CreatePinParams));
			}
...
{
	return FindPinChecked(ExecutePinName);
}
...
{
	return FindPinChecked(FormatPinName);
}

UEdGraphPin* UK2Node_SiriusPrintStringFormatted::TryFindArgumentPin(const FName PinName) const
{
	for (UEdGraphPin* Pin : ArgumentPins)
	{
		if (Pin->PinName.ToString().Equals(PinName.ToString(), ESearchCase::CaseSensitive))
		{
			return Pin;
		}
	}
...
bool UK2Node_SiriusPrintStringFormatted::IsArgumentPin(const UEdGraphPin* Pin) const
{
	return ArgumentPins.Contains(Pin);
}
